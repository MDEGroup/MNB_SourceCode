...
def predict_topics2(dirs, test_dir,  train_data, labels, num_topics):
    ###Extracting features
    print('Extracting features from dataset')
    count_vect = TfidfVectorizer(input='train', stop_words={'english'}, lowercase=True, analyzer='word')
    train_vectors = count_vect.fit_transform(train_data)
    ...
    for dir in dirs:
        ...
        out_results = open("test_format_results_5.csv", "a+", encoding="utf-8",
                           errors="ignore")
        out_results2 = open("test_format_results_2.csv", "a+", encoding="utf-8",
                           errors="ignore")
        out_results3 = open("test_format_results_8.csv", "a+", encoding="utf-8",
                            errors="ignore")

        calculate_metrics(lista,out_results,out_results2,out_results3, actual, predicted_topics, False)
